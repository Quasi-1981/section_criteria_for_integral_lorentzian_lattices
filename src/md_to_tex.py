...
# ★Повний ряд цифр, а не «ті, що траплялися»: пропущена цифра валить гейт аж на збірці
#   наступної роботи, і кожен раз це виглядає як нова несподіванка (P8: ₉, ⁰, ⁵, ⁿ, ᵢ).
SUB = {"₀": "0", "₁": "1", "₂": "2", "₃": "3", "₄": "4", "₅": "5", "₆": "6",
       "₇": "7", "₈": "8", "₉": "9", "₋": "-", "ᵢ": "i", "ₙ": "n", "ₖ": "k"}
SUP = {"⁰": "0", "¹": "1", "²": "2", "³": "3", "⁴": "4", "⁵": "5", "⁶": "6",
       "⁷": "7", "⁸": "8", "⁹": "9", "⁻": "-", "⁺": "+", "ⁿ": "n",
...
MATH = {
    ...
    # ★додано на збірці P8: нільпотентний клин і його централізатор приносять ⋉,
    #   гіперболічна межа — ∂, порожній набір уцілілих дзеркал — ∅, добуток за
    #   кратностями орбіти — ∏, об'єднання H³ з межею — ∪, скінченне поле — 𝔽.
    "∂": r"\partial", "∅": r"\emptyset", "∏": r"\prod", "∪": r"\cup",
    "⋉": r"\ltimes", "𝔽": r"\mathbb{F}",
}
...
def _inline(s):
    ...
    ★ЗІРКА З КОД-СПАНА ЕКРАНУЄТЬСЯ, і це не косметика (спіймано збіркою P8).
    Дуальна ґратка пишеться `A₄*`, і та зірка доживала до курсивного проходу
    нижче, де паруваласяз відкривальною зіркою заголовка книжки в тому самому
    рядку: «`A₄*` … *Aperiodic Order*» давало \\emph{, розірваний посеред
    математики, і pdflatex сипав «Missing $ inserted». Помилка МОВЧАЗНА в md і
    видима лише в логу TeX, тому зірка з коду ховається до плейсхолдера й
    вертається після того, як курсив уже розібрано.

    ★ХОВАЄТЬСЯ ЛИШЕ ОДИНОЧНА ЗІРКА. Перший захід ховав усі — і зняв
    легітимний **жирний** усередині код-спана, яким препринт-7 підсвічує
    значення в таблиці (`{0, **2**, 6}` → \\textbf{2} у математиці). Спіймала
    регресія G4 по вже виданій роботі: тіло .tex розійшлося на одному рядку.
    Пара «**» лишається парою; ховається тільки самотня зірка дуальної ґратки."""
    out, i = [], 0
    for m in re.finditer(r"`([^`]+)`", s):
        out.append(_txt(s[i:m.start()]))
        out.append("$" + re.sub(r"(?<!\*)\*(?!\*)", _STAR, _math(m.group(1))) + "$")
        i = m.end()
    out.append(_txt(s[i:]))
    r = "".join(out)
    r = re.sub(r"\*\*(.+?)\*\*", r"\\textbf{\1}", r)
    r = re.sub(r"(?<![\*\w])\*([^*]+?)\*(?!\*)", r"\\emph{\1}", r)
    return r.replace(_STAR, "*")
